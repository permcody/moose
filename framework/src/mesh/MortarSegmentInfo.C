...
{
  bool b1 =
      (std::abs(xi1_a) < 1. + libMesh::TOLERANCE) && (std::abs(xi1_b) < 1. + libMesh::TOLERANCE);
  bool b2 =
      (std::abs(xi2_a) < 1. + libMesh::TOLERANCE) && (std::abs(xi2_b) < 1. + libMesh::TOLERANCE);

  bool xi2a_unset = (std::abs(xi2_a - invalid_xi) < libMesh::TOLERANCE);
  bool xi2b_unset = (std::abs(xi2_b - invalid_xi) < libMesh::TOLERANCE);
  ...
  // value).
  if (std::abs(xi1_a - xi1_b) < libMesh::TOLERANCE)
  {
  ...
  if (xi2_set && (std::abs(xi2_a - xi2_b) < libMesh::TOLERANCE))
  {
...
{
  bool xi2_set = (std::abs(xi2_a - invalid_xi) >= libMesh::TOLERANCE) &&
                 (std::abs(xi2_b - invalid_xi) >= libMesh::TOLERANCE);
