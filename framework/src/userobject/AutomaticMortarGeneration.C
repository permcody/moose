...
// Local helper function used in the bodies of the loops below.
const Elem *
loop_body(const ElemSideBCTriple & triple,
          std::set<subdomain_id_type> & added_ids,
          AutomaticMortarGeneration & amg)
{
  Elem * elem = triple.elem;
  unsigned int side = triple.side;
  boundary_id_type bc_id = triple.bc_id;

  // libMesh::out << "Adding Elem " << elem->id()
  //              << "'s side " << side
  //              << " to the Mesh." << std::endl;

  // Build a non-proxy element from this side.
  UniquePtr<Elem> side_elem(elem->build_side_ptr(side, /*proxy=*/false));
  ...
  subdomain_id_type added_elem_subdomain_id =
      cast_int<subdomain_id_type>(amg.boundary_subdomain_id_offset + bc_id);
  ...
  const Elem * added_side = amg.mesh.add_elem(side_elem.release());
  ...
  amg.lower_elem_to_side_id[added_side] = side;
...
void
AutomaticMortarGeneration::add_lower_dimensional_elements()
{
  ...
  {
    if (slave_requested_boundary_ids.count(bc_id_list[i]))
      slave_sides_to_add.push_back(
          ElemSideBCTriple(mesh.elem_ptr(element_id_list[i]), side_list[i], bc_id_list[i]));

    if (master_requested_boundary_ids.count(bc_id_list[i]))
      master_sides_to_add.push_back(
          ElemSideBCTriple(mesh.elem_ptr(element_id_list[i]), side_list[i], bc_id_list[i]));
  }
  ...
  ////////////////////////////////////////////////////////////////////////////////
  {
    ...
    {
      const Elem * added_side = loop_body(triple, added_slave_subdomain_ids, *this);

      // Keep track of max lower-dimensional element size on the slave side.
      this->h_max = std::max(this->h_max, added_side->volume());
    }
  ...
  ////////////////////////////////////////////////////////////////////////////////
  {
    ...
    {
      loop_body(triple, added_master_subdomain_ids, *this);
    }
...
AutomaticMortarGeneration::compute_nodal_normals()
{
  ...
  // each one.
  // We loop over all active elements currently, but this procedure could be parallelized as well.
  for (MeshBase::const_element_iterator el = mesh.active_elements_begin(),
                                        end_el = mesh.active_elements_end();
       el != end_el;
       ++el)
    ...
    subdomain_id_type lower_dimensional_slave_subdomain_id)
{
  ...
  //     key.second->id() << std::endl;
  //     libMesh::out << "Val: xi^(1)=" << val.first << ", Slave elem id=" << val.second->id() <<
  //     std::endl;
  //   }
}
    ...
    subdomain_id_type lower_dimensional_slave_subdomain_id)
{
    ...
    for (unsigned int n = 0; n < slave_side_elem->n_nodes(); ++n)
    {
      ...
      for (unsigned r = 0; r < result_set.size(); ++r)
      {
        ...
        for (unsigned int e = 0; e < master_elem_candidates.size(); ++e)
        {
          ...
          // the master
          // element that the slave node projects to along the nodal normal. Note: we have made the
          // following simplifications to that equation in order to solve for xi:
          // 1.) Only the z (out-of-plane) component is solved for.
          // 2.) We assume linear elements.
          ...
          if (std::abs(xi2) <= 1. + TOLERANCE)
          {
            ...
            if (std::abs(std::abs(xi2) - 1.) < TOLERANCE)
            {
              ...
              // can
              // be a max of 2 nodal neighbors, and we want to make sure that the slave nodal
  ...
  //     key.second->id() << std::endl;
  //     libMesh::out << "Val: xi^(2)=" << val.first << ", Master elem id=" << val.second->id() <<
  //     std::endl;
  //   }
}
