...
void FeatureFloodCount::communicateAndMerge()
{
  ...
  if (processor_id() == 0)
  {
    ...
    std::vector<std::vector<unsigned int> > local_to_global_root(_n_procs, std::vector<unsigned int>(_feature_count));
    ...
    for (const auto & feature : _feature_sets)
    {
      ...
      for (const auto & local_index_pair : feature._orig_ids)
                             // rank                 // local index
        local_to_global_root[local_index_pair.first][local_index_pair.second] = feature_number;

      ++feature_number;
    }

    // Rank 0 broadcast for _feature_count
    _communicator.broadcast(_feature_count, 0);

    mooseAssert(local_to_global_root.size() == _n_procs, "local_to_global not sized properly");
    for (auto rank = decltype(_n_procs)(1); rank < _n_procs; ++rank)
      _communicator.send(rank, local_to_global_root[rank]);

    // Finally save off root's mapping (identity)
    _local_to_global_feature_map.resize(_feature_count);
    for (decltype(_local_to_global_feature_map.size()) i = 0, n = _local_to_global_feature_map.size(); i < n; ++i)
      _local_to_global_feature_map[i] = i;
  }

  else // Ranks 1..n
  {
    // Rank 1..n broadcast for _feature_count
    _communicator.broadcast(_feature_count, 0);
    _feature_sets.reserve(_feature_count);
...
//  decltype(FeatureFloodCount::FeatureData::_var_idx) old_var_index = 0;
   ...
   * global map and
   */
  mooseAssert(_feature_sets.size() == _local_to_global_feature_map.size(), "Local to global map not sized correctly");
  for (decltype(_feature_sets.size()) i = 0, end_index = _feature_sets.size(); i < end_index; ++i)
  {
    ...
    else
    {
      ...
      mooseAssert(local_id < _local_to_global_feature_map.size(), "local_id is out of range");
