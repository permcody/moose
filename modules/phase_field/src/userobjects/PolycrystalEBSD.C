                                       ...
                                       std::vector<unsigned int> & grains) const
{
  ...
  if (_phase && _phase != libMesh::invalid_uint && _phase != d._phase)
  {
    grains.resize(0);
    return;
  }
  ...
  grains.resize(1);
  grains[0] = _phase && _phase != libMesh::invalid_uint ? local_id : global_id;
}

unsigned int
PolycrystalEBSD::getNumGrains() const
{
  if (_phase && _phase != libMesh::invalid_uint)
...
PolycrystalEBSD::getNodalVariableValue(unsigned int op_index, const Node & n) const
{
  ...
  for (auto index = decltype(num_grains)(0); index < num_grains; ++index)
  {
    ...
    auto grain_index =
        _phase && _phase != libMesh::invalid_uint ? _ebsd_reader.getGlobalID(_phase, index) : index;
